...
$(document).ready(function() {
   ...
   $("<select />").appendTo("#navigation");
	...
	//FONT AND STYLESHEET SWITCHER -  JUST A QUICK DEVELOPMENT FOR THE DEMO :)

	// STYLESHEET SWITCHER
	
	var cooked = $.cookie('demention-skin');
	
	if ( cooked ) {
		$('#skin').attr('href', 'css/skins/' + cooked  + '.css');
	}

	$('[data-skin]').click( function () {
		var stylesheet  =  $(this).attr('data-skin');
		
		$('#skin').attr('href', 'css/skins/' + stylesheet + '.css');

		$.cookie('demention-skin', stylesheet , {path: '/'});
		return false;
	});
	...
	var cooked = $.cookie('demention-font');
	
	if ( cooked ) {
		$('#font').attr('href', 'css/fonts/' + cooked  + '.css');
	}
	
	$('[data-font]').click( function () {
		var stylesheet  =  $(this).attr('data-font');
		$('#font').attr('href', 'css/fonts/' + stylesheet + '.css');
		$.cookie('demention-font', stylesheet , {path: '/'});
		return false;
	});
